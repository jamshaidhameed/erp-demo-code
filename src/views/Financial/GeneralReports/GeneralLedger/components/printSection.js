...
export default function GLReportPrint({ data, costCentresById }) {
  const processData = (data) => {
    if (data?.length > 0) {
      return data?.map((parent) => {
        ...
        return (
          <div key={parent.OID} style={styles.parent}>
            <div style={styles.headerTitle}>General Ledger</div>
  ...
  return <>{data && <div id={elementIds.GL_REPORT_PRINT_CONTENT}>{reportRender}</div>}</>;
}
